...
def bcw_year_overlap_file_generator(
        boundaries_wave, nc_files_wave, mdw_file, start_time, end_time, step_wave, bcw_file_name, crs_type, reference_time, tstart):
    # %% Import packages
    ...
    def extract_data_for_loc(datasets, dataframe_loc, output_dict, variable):
        """
        ...
        for index, row in tqdm(dataframe_loc.iterrows(), desc=f'Extracting Data: {variable}',
            ...
            # Initialize an empty list to combine data across all datasets
            combined_data = []
    
            # Loop through each dataset in the list
            for dataset in datasets:
                # Perform bilinear interpolation for the current dataset
                interpolated_data = bilinear_interpolation(
                    row['lat'], row['lon'], dataset, variable
                )
                combined_data.append(interpolated_data)
    
            # Concatenate the data from all datasets
            dataset_combine = np.concatenate(combined_data)
    
            # Store the combined data in the output dictionary
            output_dict[row['boundaries']].append(dataset_combine)
    
    def extract_dir_data_for_loc(datasets, dataframe_loc, output_dict, variable):
        """
        Extract data for a specific variable from a list of datasets and store it in the output_dict.
        ...
        for index, row in tqdm(dataframe_loc.iterrows(), desc=f'Extracting Direction: {variable}',
            ...
            # # Replace empty values with mode (if necessary, as bilinear interpolation makes this rare)
            # for i in dataset_combine:
            #     if i == -0.017452405765652657:  # Specific empty value
            ...
            #     elif i == 0.9998477101325989:  # Another specific empty value
            #         to_calculate = dataset_combine[np.where(
    ...
    nc_files_wave = nc_files_wave
    # Ensure the nc_files list is ordered by year
    nc_files_wave.sort(key=extract_year)
    ...
    #remove all b boundary points as it is not needed for waves
    bnd_loc_geo = bnd_loc_geo[~bnd_loc_geo['boundaries'].str.endswith('_b')]
    bnd_loc_geo.reset_index(drop=True, inplace=True)
    ...
    extract_dir_data_for_loc(
        datasets=datasets,  # List of datasets
        dataframe_loc=bnd_loc_geo,  # Location DataFrame
        output_dict=extracted_x_y_dict,  # Dictionary to store results
        variable=wave_dir_x  # Variable to extract
    )
    ...
    extract_dir_data_for_loc(
        ...
        dataframe_loc=bnd_loc_geo,  # Location DataFrame
        output_dict=extracted_x_y_dict,  # Dictionary to store results
        variable=wave_dir_y  # Variable to extract
    )
    ...
    # Preallocate the dictionary for storing extracted data
    extracted_dataset_dict = {}
    for index, row in bnd_loc_geo.iterrows():
        extracted_dataset_dict[row['boundaries']] = []  # Create keys for the dict
    
    # Extract data for each variable from all datasets
    extract_data_for_loc(
        datasets=datasets,  # List of datasets
        dataframe_loc=bnd_loc_geo,  # Location DataFrame
        output_dict=extracted_dataset_dict,  # Dictionary to store results
        variable=sig_height  # Significant wave height
    )
    
    extract_data_for_loc(
        datasets=datasets,
        dataframe_loc=bnd_loc_geo,
        output_dict=extracted_dataset_dict,
        variable=peak_period  # Peak period
    )
    ...
    extract_data_for_loc(
        ...
        dataframe_loc=bnd_loc_geo,
        output_dict=extracted_dataset_dict,
        variable=dir_spread  # Directional spread
    )
